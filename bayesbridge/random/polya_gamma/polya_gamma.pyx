...
def rand_polyagamma(shape, tilt):
    ...
    for index in range(n_samples):
        for j in range(shape_view[index]):
            result_view[index] += rand_unit_shape_polyagamma(tilt_view[index])

    return result


cdef double rand_unit_shape_polyagamma(double tilt):
    return .25 * rand_tilted_jocobi(.5 * tilt)


cdef double rand_tilted_jocobi(double tilt):
    """
    ...
    while not accepted:
        X, proposal_density = rand_proposal(tilt)
        U = <double>random.random() * proposal_density
        accepted = decide_acceptability(U, X, proposal_density)

    return X


cdef (double, double) rand_proposal(double tilt):
    ...
    cdef double prob_to_right = calc_prob_to_right(tilt, exp_rate)
    if <double>random.random() < prob_to_right:
        X = rand_left_truncated_exp(1. / exp_rate, threshold)
    else:
        X = rand_right_truncated_unit_shape_invgauss(tilt, threshold)
    proposal_density = calc_next_term_in_series(0, X)
    return X, proposal_density


cdef double calc_prob_to_right(double tilt, double exp_rate):
    cdef double threshold = PIECEWISE_DENSITY_THRESHOLD
...
cdef double calc_next_term_in_series(int n, double x):
...
cdef bint decide_acceptability(double U, double X, double zeroth_term):
    ...
    while not is_determinate:
        partial_sum += sign * calc_next_term_in_series(n_summed, X)
        n_summed += 1
        if sign == -1:
            if U <= partial_sum:
                accepted = True
...
cdef double rand_left_truncated_exp(double scale, double trunc):
    return trunc - scale * log(1.0 - <double>random.random())


# Ref: "Simulation of truncated gamma variables" by Younshik Chung
# Korean Journal of Computational & Applied Mathematics, 1998
cdef double rand_left_truncated_chisq(double trunc):
    cdef double X, density_ratio
    cdef bint accepted = False
    while not accepted:
        X = rand_left_truncated_exp(2., trunc)
        density_ratio = sqrt(0.5 * M_PI / X)
        accepted = (<double>random.random() <= density_ratio)
    return X


cdef double rand_right_truncated_unit_shape_invgauss(double rate, double trunc):
    ...
    if mean > trunc:
        ...
        while not accepted:
            X = 1.0 / rand_left_truncated_chisq(.5 * M_PI)
            accepted = (log(<double>random.random()) < - 0.5 * X * rate ** 2)
    else:
        while not accepted:
            X = rand_unit_shape_invgauss(mean)
            accepted = (X < trunc)
    return X
